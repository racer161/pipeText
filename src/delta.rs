...
fn get_diff(old_text_unicode : &Vec<(usize, &str)>, new_text_unicode : &Vec<(usize, &str)>, cursor_index: usize) -> (usize, usize, isize)
{
    let delta : isize = (new_text_unicode.len() as isize) - (old_text_unicode.len() as isize);

    let limit : usize = cmp::max(0, (cursor_index as isize - delta)as usize);
    ...
    let start_limit : isize = cursor_index as isize - cmp::max(0, delta);
    while (start as isize) < start_limit && old_text_unicode[start].1 == new_text_unicode[start].1 {
        start += 1;
    }
...
pub fn get_input_edit_from_diff(old_source_code : &str, new_source_code : &str, cursor_index : usize) -> InputEdit
{
    ...
    let (start, end, delta) = get_diff(&old_text_unicode, &new_text_unicode, cursor_index);
